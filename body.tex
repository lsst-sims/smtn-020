...
lower-tier survey: lower tiers execute only when no survey in any higher tier has
a feasible observation to request. Within each tier, surveys are ranked by their
current reward values. Each \texttt{Survey} instance contains its own set of
\texttt{BasisFunction} objects, which collectively define the constraints and
optimization criteria that guide that survey's observation selection.

The tier structure and the number of surveys per tier are entirely determined by
the use case. A configuration for the full LSST survey contains on the order of
ten tiers. The highest tier holds a set of eleven \texttt{ScriptedSurvey}
instances dedicated to Target of Opportunity (ToO) observations; if a ToO is
triggered, these surveys take priority provided their constraints are met. The
second and third tiers are populated with one or two \texttt{ScriptedSurvey}
instances targeting Deep Drilling Field observations. Reward-based surveys
covering large sky areas with different cadence goals fill the remaining tiers,
with lower-priority gap-filling surveys occupying the bottom tier. By contrast, a
simpler configuration for a commissioning task or targeted observing program
might use a single tier with only one or two surveys. This flexibility allows the
same scheduling framework to serve purposes ranging from ten-year survey
simulations to short on-sky commissioning sequences.
...
\texttt{ScriptedSurvey} instances handling Deep Drilling Fields.
...
images themselves. A few examples of issues found and resolved during this
phase include:
%
\begin{itemize}
...
target of opportunity configurations (ref Sean's paper) and updating coverage for very large ToOs

downstream data management needs (templates and adding a template tier)

generating prenight simulations and longer term simulations (meeting challenge of weather and downscope)

interactions with the SCOC and the community (reporting)

\section{Conclusions}\label{sec:conclusions}

Going forward into prelsst early operations and into the future
